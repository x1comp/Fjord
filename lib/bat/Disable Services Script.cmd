...
echo off && title Service Disabler && mode con: cols=100 lines=25 && color 02 && cls
echo Disable Services Script by Snowy && timeout /t 5 >nul && cls

:main
echo Choose disabler mode: (basic/advanced) && set /p inst=
...
:prompt
echo Is "%inst%" the mode you would like to apply? && set /p prmt=
...
if /i "%inst%" == "basic" echo Basic mode succesfully applied! && timeout /t 2 >nul && goto :exit
) else (
     echo Basic mode succesfully applied! && goto :advanced
)
...
echo Advanced Mode succesfully applied! && timeout /t 2 >nul && goto :exit

:exit
cls && echo Succesfully disabled Services && timeout /t 3 >nul && exit