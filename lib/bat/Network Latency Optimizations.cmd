...
cls
echo Batch script created by Snowy!
timeout 3 > nul
...
echo.
timeout 2 > nul
echo Nagle's Algorithm is used to improve the efficiency of TCP networks by 
echo decresing the number of packets that needs to be sent over the network, by allowing 
echo several small packets to be combined together into a single yet larger packet for 
echo more efficient transmissions. Nagle's Algorithm it has been proven to increase latency 
...
echo.
timeout 8 > nul
echo I would advocate on disabling Nagle's Algorithm as in testing as
echo it helps extremly with reducing network latency.
timeout 2 > nul
echo.
...
echo number of outstanding ACKs before ignoring delayed ACK timer). For the best gaming performance, the recommended value is 1 
echo (disabled). For pure throughput(maximum UP/DOWN Speeds) and data streaming, you can experiment with values over 2. Wifi 
echo may see a slight improvement with disabled TcpAckFrequency as well.
echo.
echo.
...
echo The "Multimedia Class Scheduler" service (MMCSS) is used to ensure prioritized access 
echo to CPU resources, without excluding CPU resources to lower-priority background 
...
echo help some games and video streaming. I recommend reducing the reserved CPU for 
...
echo millisecond (ms). The idea behind implementing a network throttle is because processing network 
echo packets can be a resource-intensive task, and it may need to be throttled to 
echo give prioritized CPU access to games and othe multimedia programs. In some cases, such as 
echo Gigabit networks and some online games, for example, it is recommended to turn 
echo off such throttling all together to reach maximum throughput.
echo.
timeout 2 > nul
echo Would you like to enable SystemProfile optimizations?
set /p a=
